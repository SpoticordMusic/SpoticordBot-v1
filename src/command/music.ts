import { Client, Message, MessageEmbed, TextChannel, MessageButton, MessageActionRow, Interaction, ButtonInteraction, VoiceChannel } from "discord.js";
...
export default class MusicCommands {
    ...
    async join(event: CommandEvent) {
        ...
        const botPerms = event.msg.guild.members.cache.get(event.msg.client.user.id).permissionsIn(event.msg.member.voice.channel);
        ...
        if (this.music.getPlayerState(event.msg.guild.id) === 'DISCONNECTED') {
            await this.music.joinChannel(event.msg.guild.id, event.msg.member.voice.channel as VoiceChannel, <TextChannel>event.msg.channel);
    ...
    private buildPlayingEmbed(guild: string, host: string) {
        ...
        positionText = `${isPaused ? '⏸️' : '▶️'} ${positionText} ${_strtime(Math.floor(player_info.position / 1000))} / ${_strtime(Math.floor(player_info.youtube_track.duration / 1000))}`;

        const prev_button = new MessageButton()
            .setStyle('PRIMARY')
            .setLabel('<<')
            .setCustomId('btn_previous_track');

        const pause_resume_button = new MessageButton()
            .setStyle('SECONDARY')
            .setLabel(isPaused ? 'Play' : 'Pause')
            .setCustomId('btn_pause_play');

        const next_button = new MessageButton()
            .setStyle('PRIMARY')
            .setLabel('>>')
            .setCustomId('btn_next_track');

        const actionRow = new MessageActionRow()
            .addComponents(prev_button, pause_resume_button, next_button);
    ...
    async previousTrack(interaction: ButtonInteraction) {
        const player = this.music.getPlayer(interaction.guild.id);
        if (!player) await interaction.deleteReply();
        if (!player.getHost() || this.music.getPlayerState(interaction.guild.id) === 'DISCONNECTED' ||
            this.music.getPlayerState(interaction.guild.id) === 'INACTIVE') return await interaction.deferUpdate();
        ...
        if (await player.seek(0)) await this.updatePlayingMessage(interaction);
    }

    // Pause or resume track button clicked
    async pausePlayTrack(interaction: ButtonInteraction) {
        const player = this.music.getPlayer(interaction.guild.id);
        if (!player) await interaction.deleteReply();
        if (!player.getHost() || this.music.getPlayerState(interaction.guild.id) === 'DISCONNECTED' ||
            this.music.getPlayerState(interaction.guild.id) === 'INACTIVE') return await interaction.deferUpdate();
        ...
        if (player.getPlayerInfo().paused) {
            if (await player.resume()) await this.updatePlayingMessage(interaction);
        } else {
            if (await player.pause()) await this.updatePlayingMessage(interaction);
        }
    }

    // Next track button clicked
    async nextTrack(interaction: ButtonInteraction) {
        const player = this.music.getPlayer(interaction.guild.id);
        if (!player) await interaction.deleteReply();
        if (!player.getHost() || this.music.getPlayerState(interaction.guild.id) === 'DISCONNECTED' ||
            this.music.getPlayerState(interaction.guild.id) === 'INACTIVE') return await interaction.deferUpdate();
        ...
        if (await player.next()) {
            ...
            await this.updatePlayingMessage(interaction, true);
        }
    }

    async updatePlayingMessage(button: ButtonInteraction, dont_defer=false) {
        try {
            ...
            try { await button.deferUpdate(); } catch {}
        }
    }
    ...
    attachButtonHandlers(client: Client) {
        client.on('interactionCreate', async (interaction) => {
            ...
            if (interaction.customId === 'btn_previous_track') await this.previousTrack(interaction);
            if (interaction.customId === 'btn_pause_play') await this.pausePlayTrack(interaction);
            if (interaction.customId === 'btn_next_track') await this.nextTrack(interaction);
        });
    }
}
