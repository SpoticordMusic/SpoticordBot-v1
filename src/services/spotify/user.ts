...
export class SpotifyUser extends EventEmitter {
  ...
  private state: "INITIALIZED" | "ACTIVE" = "INITIALIZED";
  ...
  protected async wsOnMessage(event: WebSocket.MessageEvent) {
    ...
    if (!this.connection_id) {
      if (data.type === "message" && data.method === "PUT" && data.headers["Spotify-Connection-Id"]) {
        ...
        this.state_manager = new SpotifyStateManager(
          this.device_id,
          response.data.initial_seq_num,
          this.token,
          this.client_id,
          this.client_secret
        );
    ...
    for (const payload of data.payloads) {
      if (payload.type === "set_volume") {
        await axios.put(
          `https://api.spotify.com/v1/track-playback/v1/devices/${this.device_id}/volume`,
          {
            seq_num: null,
            command_id: "",
        ...
        let position = 0;
        if (this.state_manager.isSeeking()) position = this.state_manager.getSeek();
  ...
  public async setSpoticordDevice(): Promise<boolean> {
    if (!this.initialized || this.destroyed) return false;

    return await this.state_manager.setActiveDevice([this.device_id]);
  }
...
export class SpotifyWebHelper {
  static async checkScope(token: DBToken): Promise<boolean> {
    ...
    if (response.status >= 400) {
      if (!(await this.refreshAccessToken(token))) {
        await Spoticord.database.deleteToken(token.discord_id);
  ...
  static async refreshAccessToken(token: DBToken): Promise<boolean> {
    ...
    await Spoticord.database.updateAccessToken(token.discord_id, access_token);
